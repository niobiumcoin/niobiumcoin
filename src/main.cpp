...
const string strMessageMagic = "Niobium Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectNiobiumFee, bool ignoreFees)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejectNiobiumFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : niobium fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejectNiobiumFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejectNiobiumFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : niobium fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejectNiobiumFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejectNiobiumFee, ignoreFees);
}
...
{
    int64_t ret = blockValue * 0.50; // 50%
    if(randreward() <= 8000) // 5% Chance of superblock correct
