...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNiobiumAmount"))
        settings.setValue("nAnonymizeNiobiumAmount", 1000);
    nAnonymizeNiobiumAmount = settings.value("nAnonymizeNiobiumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeNiobiumAmount"))
        SoftSetArg("-anonymizeNiobiumamount", settings.value("nAnonymizeNiobiumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeNiobiumAmount:
            return QVariant(nAnonymizeNiobiumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeNiobiumAmount:
            nAnonymizeNiobiumAmount = value.toInt();
            settings.setValue("nAnonymizeNiobiumAmount", nAnonymizeNiobiumAmount);
            emit AnonymizeNiobiumAmountChanged(nAnonymizeNiobiumAmount);
            break;
        default:
            break;
        }
