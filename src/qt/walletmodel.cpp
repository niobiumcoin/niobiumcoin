...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CNiobiumAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CNiobiumAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject niobium fee
        unsigned int niobiumfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        niobiumfee = ((MIN_RELAY_TX_FEE * niobiumfee) * 10000);
        if (nFeeRequired > niobiumfee){
            LogPrintf("nFeeRequired: %d -- NiobiumFee: %d\n", nFeeRequired, niobiumfee);
            return NiobiumFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CNiobiumAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CNiobiumAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CNiobiumAddress(address).ToString());
...
{
    // Niobium context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CNiobiumAddress(address).ToString())].push_back(out);
    }
}
