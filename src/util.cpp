...
int nAnonymizeNiobiumAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nb
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nb
    // Mac: ~/Library/Application Support/Nb
    // Unix: ~/.Nb
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nb";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Niobium.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Niobium.conf";
               ...
               fprintf(ConfFile, "port=55550\n");
               fprintf(ConfFile, "rpcport=55551\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=seed.niobiumcoin.org\n");
               fprintf(ConfFile, "addnode=seed1.niobiumcoin.org\n");
               fprintf(ConfFile, "addnode=seed2.niobiumcoin.org\n");
               fclose(ConfFile);
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Niobiumd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Niobium will not work properly.");
